...
const SimpleForm = ({ faculty, onClose, onSubmit, userId }) => {
  ...
  // State สำหรับจัดการ Email Status
    ...
    try {
      ...
      if (savedRequestId) {
        ...
        sendRequestNotificationEmail(newRequest, savedRequestId)
          .then(result => {
            if (result.success) {
              console.log('✅ ส่งอีเมลแจ้งเตือนสำเร็จ:', result);
              setEmailStatus('success');
              // แสดง alert เมื่อส่งอีเมลสำเร็จ
              alert(`✅ บันทึกข้อมูลสำเร็จ!\n\n📧 ส่งอีเมลแจ้งเตือนไปยัง hatwst1@gmail.com แล้ว\n\nRequest ID: ${savedRequestId}`);
            } else {
              console.warn('⚠️ ไม่สามารถส่งอีเมลได้:', result.message);
              setEmailStatus('error');
              // แสดง alert แจ้งเตือนว่าส่งอีเมลไม่สำเร็จ แต่ข้อมูลถูกบันทึกแล้ว
              alert(`✅ บันทึกข้อมูลสำเร็จ!\n\n⚠️ แต่ไม่สามารถส่งอีเมลได้: ${result.message}\n\nกรุณาตรวจสอบ EmailJS Configuration`);
            }
          })
          .catch(error => {
            ...
            setEmailStatus('error');
            // แสดง alert แจ้งเตือนว่าส่งอีเมลไม่สำเร็จ แต่ข้อมูลถูกบันทึกแล้ว
            alert(`✅ บันทึกข้อมูลสำเร็จ!\n\n⚠️ แต่เกิดข้อผิดพลาดในการส่งอีเมล\n\nกรุณาตรวจสอบ EmailJS Configuration และ Console Logs`);
          });
      }
